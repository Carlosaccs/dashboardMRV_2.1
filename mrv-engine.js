let DADOS_PLANILHA = [];
let pathAtivo = null;    // Para rastrear o ID do mapa selecionado
let imovelAtivo = null;  // Para rastrear o Nome do residencial selecionado
let nomeSelecionado = ""; 
let mapaAtivo = 'GSP'; 
...
// DEFINIÇÃO DA LISTA LATERAL (Adicionando ID único para facilitar o destaque)
function gerarListaLateral() {
    ...
    container.innerHTML = DADOS_PLANILHA.map(item => {
        ...
        // Criamos um ID seguro para o botão usando o nome do residencial
        const idBtn = `btn-${item.nome.replace(/\s+/g, '-')}`;
...
function renderizarNoContainer(id, dados, interativo) {
    ...
    const pathsHtml = dados.paths.map(p => {
        ...
        // Verifica se já deve iniciar com a classe ativo
        const ativo = (pathAtivo === idPathNormalizado && interativo) ? 'ativo' : '';
...
// CORREÇÃO CIRÚRGICA DO DESTAQUE LARANJA
function comandoSelecao(idPath, nomePath, fonte) {
    ...
    if (imoveis.length > 0) {
        ...
        // 1. Atualizar Variáveis de Estado
        pathAtivo = idBusca;
        imovelAtivo = selecionado.nome;

        // 2. Limpeza Visual (remove o laranja de todo mundo)
        document.querySelectorAll('.ativo').forEach(el => el.classList.remove('ativo'));

        // 3. Aplicar Destaque no Mapa
        const elMapa = document.getElementById(`caixa-a-${idBusca}`);
        if (elMapa) elMapa.classList.add('ativo');

        // 4. Aplicar Destaque na Lista Lateral (usando o ID que criamos)
        const idBtn = `btn-${selecionado.nome.replace(/\s+/g, '-')}`;
...
function limparInterface() {
    nomeSelecionado = ""; pathAtivo = null; imovelAtivo = null;
