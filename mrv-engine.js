...
function obterHtmlEstoque(valor, tipo) {
    ...
    if (!isNaN(num)) return `<span class="badge-estoque" style="color:${num < 6 ? '#e31010' : '#666'}; font-size:9px; font-weight:bold;">RESTAM ${num} UN.</span>`;
    return `<span class="badge-estoque" style="color:#666; font-size:9px;">${clean || "CONSULTAR"}</span>`;
}
...
function navegarVitrine(nome, nomeRegiao) { 
    ...
    if (!mapaContexto.paths.some(p => p.id.toLowerCase().replace(/\s/g, '') === imovel.id_path)) trocarMapas(false);
...
function comandoSelecao(idPath, nomePath, fonte) {
    ...
    gerarListaLateral(); // Atualiza a lista para marcar o ativo e mostrar estoque
...
function desenharMapas() {
    ...
    // Reatribui o clique para o mapa de baixo trocar
...
function montarVitrine(selecionado, listaDaCidade, nomeRegiao) {
    ...
    if (selecionado.tipo === 'R') {
        ...
        html += `<div style="padding: 6px 0;"><p style="font-size:0.65rem; color:#444; display:flex; justify-content:space-between;"><span>📍 ${selecionado.endereco}</span><a href="${urlMaps}" target="_blank" class="btn-maps">MAPS</a></p></div>`;
        ...
        html += `<div class="box-argumento"><label>Plantas</label><strong>${selecionado.p_de} - ${selecionado.p_ate}</strong></div>`;
        html += `<div style="display:grid; grid-template-columns:1fr 1fr; gap:4px;">
        ...
        html += `<div class="box-argumento" style="display:block !important;"><label>Sobre o Complexo</label><p style="margin-top:5px;">${selecionado.descLonga}</p></div>`;
    }
